<?php

namespace boundstate\eventful\enums;

/**
 * Methods for iCalendar (VEVENT) calendar components.
 *
 * @see https://datatracker.ietf.org/doc/html/rfc2446#section-3.2
 */
enum IcsMethod: string
{
    /**
     * Post notification of an event. Used primarily as
     * a method of advertising the existence of an event.
     */
    case PUBLISH = 'PUBLISH';
    /**
     * Make a request for an event. This is an explicit
     * invitation to one or more "Attendees". Event
     * Requests are also used to update or change an
     * existing event. Clients that cannot handle
     * REQUEST may degrade the event to view it as an
     * PUBLISH.
     */
    case REQUEST = 'REQUEST';
    /**
     * Reply to an event request. Clients may set their
     * status ("partstat") to ACCEPTED, DECLINED,
     * TENTATIVE, or DELEGATED.
     */
    case REPLY = 'REPLY';
    /**
     * Add one or more instances to an existing event.
     */
    case ADD = 'ADD';
    /**
     * Cancel one or more instances of an existing
     * event.
     */
    case CANCEL = 'CANCEL';
    /**
     * A request is sent to an "Organizer" by an
     * "Attendee" asking for the latest version of an
     * event to be resent to the requester.
     */
    case REFRESH = 'REFRESH';
    /**
     * Counter a REQUEST with an alternative proposal,
     * Sent by an "Attendee" to the "Organizer".
     */
    case COUNTER = 'COUNTER';
    /**
     * Decline a counter proposal. Sent to an
     * "Attendee" by the "Organizer".
     */
    case DECLINECOUNTER = 'DECLINECOUNTER';
}
